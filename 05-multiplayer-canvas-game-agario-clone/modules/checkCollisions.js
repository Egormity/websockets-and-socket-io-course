module.exports.checkForOrbCollisions = (pData, pConfig, orbs, settings) => {
	// ORB COLLISIONS
	for (let i = 0; i < orbs.length; i++) {
		...
		if (
			...
			distance = Math.sqrt(
				(pData.locX - orb.locX) * (pData.locX - orb.locX) + (pData.locY - orb.locY) * (pData.locY - orb.locY)
			);
			if (distance < pData.radius + orb.radius) {
				...
				if (pConfig.zoom > 1) pConfig.zoom -= 0.001; // update zoom so player doesn't get to big for screen

				pData.radius += 0.05; // increase player size
				// if (pConfig.speed < -0.001) pConfig.speed += 0.001; //increase player speed
				if (pConfig.speed > settings.defaultPlayerSpeed / 3) pConfig.speed -= 0.001;
...
module.exports.checkForPlayerCollisions = (pData, pConfig, players, playersForUsers, playerId) => {
	// PLAYER COLLISIONS
	for (let i = 0; i < players.length; i++) {
		const p = players[i];
		if (p.socketId && p.socketId != playerId) {
			// Added p.socketId test in case player has been removed from players
			let pLocx = p.data.locX;
			let pLocy = p.data.locY;
			let pR = p.data.radius;
			...
			if (
				...
				if (distance < pData.radius + pR) {
					...
					if (pData.radius > pR) {
						// ENEMY DEATH
						pData.score += p.data.score + 10;
						pData.playersAbsorbed += 1;
						p.alive = false;
						pData.radius += p.data.radius * 0.25;

						const collisionData = {
							absorbed: p.data.name,
							absorbedBy: pData.name,
						};
						if (pConfig.zoom > 1) pConfig.zoom -= pR * 0.25 * 0.001;
